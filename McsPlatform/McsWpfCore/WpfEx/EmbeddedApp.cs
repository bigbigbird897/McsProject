namespace McsCoreLib.Tools
{
    ...
    {
        internal class EmbeddedApp : HwndHost, IKeyboardInputSink
        {
            ...
            private readonly double screenW;
            private readonly double screenH;
            ...
            protected override HandleRef BuildWindowCore(HandleRef hwndParent)
            {
                ...
                appProc.Start();
                hwndHost = FindTheWindow();
                _ = SendMessage(hwndHost, WM_SYSCOMMAND, SC_MINIMIZE, 0);
                //}
                ...
                newStyle &= ~WS_POPUP;
                newStyle &= ~WS_BORDER;
                _ = SetWindowLong(hwndHost, GWL_STYLE, newStyle);
                //将窗口居中，实际上是将窗口的容器居中
                RePosWindow(WndHoster, WndHoster.Width, WndHoster.Height);

                //将netterm的父窗口设置为HwndHost
                _ = SetParent(hwndHost, hwndParent.Handle);
                return new HandleRef(this, hwndHost);
            }

            protected override void DestroyWindowCore(HandleRef hwnd)
            {
                _ = SetWindowLong(hwndHost, GWL_STYLE, oldStyle);
                _ = SetParent(hwndHost, (IntPtr)0);
                appProc.Kill();
                appProc.Close();
            }
            ...
            public static bool CallBackProc(IntPtr hwnd, ref WindowInfo WndInfoRef)
            {
                ...
                _ = GetWindowText(hwnd, str, str.Capacity);
                Console.WriteLine(str.ToString());
                if (str.ToString().IndexOf(WndInfoRef.winTitle, 0) >= 0)
                {
                    WndInfoRef.hwnd = hwnd;
                    _ = GetWindowRect(hwnd, ref (WndInfoRef.r));
                }
            ...
            public IntPtr FindTheWindow()
            {
                ...
                _ = EnumWindows(callBackPtr, ref WinInfo);
                return WinInfo.hwnd;
            }
